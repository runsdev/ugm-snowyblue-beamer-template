...
\begin{frame}
    \frametitle{Advanced Overlay Examples}
    
    \onslide<1->
    First, we introduce the basic concept.
    
    \onslide<2->
    Then, we add more details to our explanation.
    
    \begin{center}
        \includegraphics<1>[width=0.4\textwidth]{example-image-a}
        \includegraphics<2>[width=0.4\textwidth]{example-image-b}
        \includegraphics<3>[width=0.4\textwidth]{example-image-c}
    \end{center}
    
    \onslide<3->
    Finally, we reach our conclusion with all the evidence.
    
    \begin{tcolorbox}[colback=ugmLightGrey,colframe=ugmBlue,title=Advanced Techniques]
        \begin{itemize}
            \item Use \texttt{\textbackslash onslide<2->} for paragraphs
            \item Apply overlay specs to any command: \texttt{\textbackslash includegraphics<2>}
            \item Use \texttt{\textbackslash only<2>{content}} to completely hide content
            \item Use \texttt{\textbackslash alt<2>{option 1}{option 2}} for alternatives
        \end{itemize}
    \end{tcolorbox}
\end{frame}
